...
const user = () => {
    do {
        userName = prompt("What's your name? This will affect the name in our rankings.");
...
const currentRanking = () => {    
    ...
    for (let i = 0 ; i < rankingList.length ; i++) {
        rankingMessage.push(`${i+1}: ${rankingList[i]['points']} pts. - ${rankingList[i]['player']} did bingo in round ${rankingList[i]['finishingRound']}.`);
    }
...
const cardboard = () => {
    ...
    do {
        ...
        if (question === 'no') {
            alert('The bingo will begin, take a seat.');
        }
...
const cardboardGenerator = () => {
    ...
    for (let i = 0 ; i < 15 ; i++) {
        ...
        let numToAdd = randomNumberGenerator();
        let check = numsInSingleArray.includes(numToAdd);
...
const cardboardQuestion = () => {
    do {
        question = prompt("Would you prefer another cardboard?\nType 'yes/no' to continue.");
...
function rotatingDrum() {
    ...
    do {
        ...
        let ball = randomNumberGenerator();
        ...
        for (let i = 0 ; i < nums.length ; i++) {
            ...
            if (check === true) {
                let indexBall = nums[i].indexOf(ball);
                nums[i][indexBall] = 'X';
                break;
            }
        }
...
const checkLine = (val) => {
    ...
    for (let i = 0 ; i < nums[val].length ; i++) {
        ...
        if (marked === 5) {
            ...
            alert(`Congrats ${userName}, you filled a LINE!!!`);
            lineFilled = true;
        }
    }
}
    ...
    for (let i = 0 ; i < nums.length ; i++) {
        for (let j = 0 ; j < nums[i].length ; j++) {
            ...
            if (marked === 15) {
                ...
                alert(`Congrats ${userName}, you did BINGO!!!`);
                bingoRound = count;
                bingoFilled = true;
            }
        }
...
const ask = () => confirm("Do you want to keep going?")? answ = true : answ = false ;

const updatedRanking = () => {    
    rankingList.push({'player': userName, 'finishingRound': bingoRound, 'points': (90 - bingoRound) * 4});
    rankingList.sort((a,b) => b.points - a.points);

    rankingMessage = [`The updated ranking is:`];
    for (let i = 0 ; i < rankingList.length ; i++) {
        rankingMessage.push(`${i+1}: With ${rankingList[i]['points']} pts. ${rankingList[i]['player']} did bingo in round ${rankingList[i]['finishingRound']}.`);
    }

    console.log(`You filled your entire cardboard in round n.${bingoRound}, achieving a total of ${(90-bingoRound)*4}pts.`);
    alert(`You filled your entire cardboard in round n.${bingoRound}, achieving a total of ${(90-bingoRound)*4}pts.`);
    console.log(rankingMessage.join('\n'));
    alert(rankingMessage.join('\n'));
